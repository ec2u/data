...
public interface EuroSciVoc extends Taxonomy {
    ...
    EuroSciVocFrame TAXONOMY=new EuroSciVocFrame();
    ...
    String EXTERNAL="http://data.europa.eu/8mn/euroscivoc/";
    String INTERNAL=EUROSCIVOC+"/";
    ...
    static void main(final String... args) { // !!! refactor
        exec(() -> service(store()).partition(EUROSCIVOC).update(array(list(Xtream
                        ...
                        Xtream.of(URL)
                                ...
                                .optMap(concept -> concept.uri().map(id -> new TopicFrame()
                                        ...
                                        .definition(concept.forward(SKOSXL.PREF_LABEL).forward(SKOSXL.LITERAL_FORM)
                                                .texts(LOCALES)
                                                .map(map -> map.get(EN))
                                                .flatMap(label -> service(Analyzer.analyzer())

                                                        // !!! look for an existing definition in the graph before recomputing

                                                        .prompt("""
                                                                Create a definition between 250 and 500 chars for the research activity related to to the given topic in the European Science Vocabulary (EuroSciVoc) taxonomy of fields of science based on OECD's 2015 Frascati Manual taxonomy.
    ...
    private static URI adopt(final URI id) {
