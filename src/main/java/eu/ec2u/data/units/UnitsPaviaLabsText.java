...
public final class UnitsPaviaLabsText implements Runnable {
    ...
    public static void main(final String... args) {
        exec(() -> new UnitsPaviaLabsText().run());
    }
    ...
    public void run() {
        ...
        departments()
                ...
                .filter(page -> page.type().equals("directory"))
                .limit(5)

                .forEach(page -> System.out.printf("%s %s %s\n", page.type(), page.url(), page.title()));
    ...
    private Xtream<Page> pages(final Department department) {
        return Xtream.of(department.homepage())

                .flatMap(new Crawl())

                .flatMap(url -> Xtream.of(url)
                        .optMap(new GET<>(new HTML()))
                        .flatMap(document -> Xtream.of(document)
                                .map(XPath::new)
                                .optMap(xml -> xml.string("//head/meta[@property='og:title']/@content")
                                        .or(() -> xml.string("//head/title"))
                                )
                                .flatMap(title -> Xtream.of(document)
                                        .map(new Untag())
                                        .optMap(content -> type(content)
                                                .map(type ->
                                                        new Page(department, url, type, title, content)
                                                )
                                        )
                                )
                        )
    ...
    private Optional<String> type(final String content) {
        return Optional.of(content)

                .flatMap(analyzer.prompt("""
                        Classify the provided markdown page from a university department web site,
                        according to the following options:
                        
                        - group (research group, team, or laboratory)
                        - directory (directory or catalog of research units)
                        - other (not otherwise classified)
                        
                        Use exclusively the given options.
                ...
                .map(JSONPath::new)
                .flatMap(json -> json.string("type"));
    }

    private Xtream<Page> xxx(final Department department) {

        final List<Page> pages=pages(department)
